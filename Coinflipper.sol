...
/// @title CoinFlipper - Testproject that shows the use of Chainlink VRF
...
contract CoinFlipper is Ownable, VRFConsumerBase {
    ...
    /* Internal functions */
    /// @dev Used by public functions to pay out both users and contract owner
    /// @param _amount Amount that is paid.
    /// @param _to Recipient's address.
    function _payout(uint256 _amount, address payable _to) 
        internal
    {
    ...
    /// @param requestId request Id that allows connecting the result to the correct user.
    /// @param randomness random number from oracle.
    ...
    /// @dev Randomizer for testing in Ganache
    //function testRandom()
    ...
    /// @dev Takes the user's choice and creates a new requestId by calling Chainlink
    /// @param choice 0-heads | 1-tails.
    /// @return Returns request ID.
    function flipCoin(uint256 choice)
        public payable costs(0.01 ether) returns (bytes32)
    {
        ...
        /// @dev Call randomizer here for testing in Ganache
        ...
        /// @dev Trim the number down to 0 or 1 for testing
    ...
    /// @dev Requests randomness from seed provided by the coinflipper contract
    /// @param userProvidedSeed seed generated through block.timestamp.
    /// @return Returns request ID.
    ...
    /// @dev Manages the result of the oracle call.
    /// @param requestId request Id that allows connecting the result to the correct user.
    /// @param randomNumber random number from oracle.
    function handleResult(bytes32 requestId, uint256 randomNumber) public {
        ...
        if(returnedUser.choice == randomNumber) {
            ...
            jackpot = jackpot - win;//remove funds from jackpot
            userBalances[returnedUser.player] = userBalances[returnedUser.player] + win;
    ...
    /// @dev User have to withdraw. Pull vs. Push logic
    ...
    /// @dev Initial funding of jackpot.
    ...
    /// @dev Owner can only withdraw the entire jackpot.
